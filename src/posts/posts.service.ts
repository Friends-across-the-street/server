...
export class PostsService {
  ...
  async getPage(
    ...
    const result = [];
    let queryStr = `SELECT p.id AS postId, p.title, p.content, p.view, p.recommend, p.created_date AS postCreateDate, p.updated_date AS postUpdateDate, u.id AS userId, u.name AS name, u.image, u.type AS userType, co.name AS companyName, j.name AS jobDescription, sc.name AS school, m.name AS major
    FROM posts AS p
  ...
  async getDetailOnePost(postId: number, user: UserDataInAuthGuard) {
    ...
    SELECT p.id AS id, p.title AS title, p.content AS content, p.view AS view, p.recommend AS recommend, p.created_date AS createdDate, p.updated_date AS updatedDate, p.user_id AS postUserId, u.name, u.image, u.type AS userType, co.name AS companyName, j.name AS jobDescription, sc.name AS school, m.name AS major
    FROM posts AS p
    LEFT JOIN users AS u ON p.user_id = u.id
    LEFT JOIN incumbents_additional AS i ON u.id = i.user_id
    LEFT JOIN company AS co ON i.company_id = co.id
  ...
  private async findCommentsByPostId(postId: number, userId: number) {
    const comments: commentsInPostForQuery[] = await this.prismaService
      .$queryRaw`
    SELECT c.id AS id, c.content, c.recommend, c.parent_comment_id AS parentCommentId, c.created_date AS createdDate, c.updated_date AS updatedDate, c.user_id AS commentUserId, c.is_delete AS isDelete, u.name AS name, u.image AS image, u.type AS userType, co.name AS companyName, j.name AS jobDescription, sc.name AS school, m.name AS major
    FROM comments AS c
